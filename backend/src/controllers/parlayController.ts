...
function renderMlbOddsHtml(events: ParlayEventOdds[], meta: { sandbox: boolean }): string {
  const rows = events
    .map((event) => {
...
<head>
  ...
  <title>MLB Odds · Parlay API</title>
  <style>
...
<body>
  <h1>MLB odds</h1>
  <p class="meta">
    Sport key: <code>${MLB_SPORT_KEY}</code>
  ...
  <p class="links">
    ...
    <a href="/api/parlay/mlb/events">MLB events</a>
  </p>
  ${rows || '<p class="muted">No MLB games with odds right now.</p>'}
</body>
</html>`;
}
...
export async function getMlbOdds(req: Request, res: Response): Promise<void> {
  try {
    const regions = typeof req.query.regions === "string" ? req.query.regions : undefined;
    const markets = typeof req.query.markets === "string" ? req.query.markets : undefined;
    const bookmakers =
    ...
    const { events, meta } = await fetchMlbOdds({ regions, markets, bookmakers });
    ...
    if (wantsHtml) {
      res.type("html").send(renderMlbOddsHtml(events, meta));
      return;
    }
    ...
    res.json({
      ...
      meta,
      links: {
        html: "/api/parlay/mlb/odds?format=html",
        events: "/api/parlay/mlb/events",
        sports: "/api/parlay/sports",
      },
    });
  } catch (err) {
...
export async function getMlbEvents(_req: Request, res: Response): Promise<void> {
  try {
    const { events, meta } = await fetchMlbEvents();
    res.json({
      sportKey: MLB_SPORT_KEY,
      sportTitle: "MLB",
      count: events.length,
